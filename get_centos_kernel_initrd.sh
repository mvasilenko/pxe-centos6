...
# auto=true priority=critical vga=788 initrd=ubuntu-installer/amd64/initrd.gz preseed/url=tftp://192.168.1.91/ubuntu-16.04-preseed.cfg preseed/interactive=false
	...
	echo "LABEL Centos_$release_$arch 
	MENU LABEL Centos_$release_$arch
	KERNEL centos/$release/$arch/linux
	APPEND initrd=centos/$release/$arch/initrd.img inst.repo=$centos_install_url/centos/$release/$arch ks=$centos_install_url/centos-$release-$arch.cfg  ksdevice=bootif
